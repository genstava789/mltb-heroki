...
async def get_buttons(key=None, edit_type=None):
    ...
    if key is None:
        ...
        if edit_type == "botvar":
            ...
            if key not in ["TELEGRAM_HASH", "TELEGRAM_HASH_PREMIUM", "TELEGRAM_API", "TELEGRAM_API_PREMIUM", "OWNER_ID", "BOT_TOKEN"]:
            ...
            buttons.ibutton("Close", "botset close")
            if key == "newser":
                msg = "Send one server as dictionary, like in config.env. Timeout: 60 sec"
            else:
...
async def edit_bot_settings(client, query):
    ...
    if data[1] == "close":
        ...
        if data[2] in default_values:
            ...
            await create_subprocess_exec(["pkill", "-9", "-f", "avaj"])
